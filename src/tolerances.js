...
export const TOLERANCES = {
    ...
    "Angle_Squareness": {
        "Data": [
            { "min": 0, "max": 30, "tolerance": 0.4 },
            { "min": 30, "max": 50, "tolerance": 0.7 },
            { "min": 50, "max": 80, "tolerance": 1.0 },
            { "min": 80, "max": 120, "tolerance": 1.4 },
            { "min": 120, "max": 180, "tolerance": 2.0 },
            { "min": 180, "max": 240, "tolerance": 2.6 },
            { "min": 240, "max": 300, "tolerance": 3.1 },
            { "min": 300, "max": 400, "tolerance": 3.5 }
        ]
    }
};

export const TOLERANCE_TYPES = [
    { id: 'DIMENSION', name: 'ÖLÇÜ' },
    { id: 'ANGLE', name: 'AÇI' }
    //{ id: 'STRAIGHTNESS', name: 'DOĞRUSALLIK' },
    //{ id: 'CONTOUR', name: 'KONTÜR' },
    //{ id: 'CORNER_RADIUS', name: 'KÖŞE RADYUSU' }
];
...
/**
 * Calculates Angle Tolerance
 * @param {number} lengthW - Length of the short side (W) in mm
 * @param {boolean} isRightAngle - True if the angle is 90 degrees (Squareness), False for other angles (Angularity)
 * @returns {number|null} - The calculated tolerance in mm
 */
export function calculateAngleTolerance(lengthW, isRightAngle) {
    if (isRightAngle) {
        // Squareness (Table 13)
        const table = TOLERANCES.Angle_Squareness;
        const range = table.Data.find(r => lengthW > r.min && lengthW <= r.max);
        if (range) {
            return range.tolerance;
        } else if (lengthW > 400) {
            // Factor for > 400? Table usually ends or has formula. 
            // Standard EN 755-9 Table 13 usually goes up to ~400 or has a factor.
            // For now return null or max? Let's return null if out of range.
            return null;
        }
        return null;
    } else {
        // Angularity (Table 14 implies factor)
        // Rule: Tolerance = W * sin(1 degree)
        // sin(1 deg) approx 0.017452406
        const factor = Math.sin(1 * Math.PI / 180);
        return lengthW * factor;
    }
}
